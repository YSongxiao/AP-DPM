...
class DualPathSwinUMamba(nn.Module):
    def __init__(
        ...
        self.out_layers_1 = nn.ModuleList()
        for i in range(4):
            self.out_layers_1.append(UnetOutBlock(
                spatial_dims=spatial_dims,
                in_channels=self.feat_size[i],
                out_channels=self.overlap_out_chans
            ))
...
class DualPathMergerSwinUMamba(nn.Module):
    def __init__(
        ...
        self.merger = UnetPlusPlus(spatial_dims=2, in_channels=in_chans+out_chans*2, out_channels=out_chans, features=(32, 32, 64, 128, 256, 32))
...
class TriplePathMergerSwinUMamba(nn.Module):
    def __init__(
        ...
        self.out_layers_2 = nn.ModuleList()
        for i in range(4):
            self.out_layers_2.append(UnetOutBlock(
                spatial_dims=spatial_dims,
                in_channels=self.feat_size[i],
                out_channels=self.overlap_sm_out_chans
            ))
